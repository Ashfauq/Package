...
class Newfile():    
    def __init__(self,filename = "NO NAME.csv",data = 0,sent_col =0,content_col = 0,sheet = "sheet 1",ascii = True ):
...
#            self.data["p_content"] = self.data["p_content"]
    ...
    def theme_cluster3(self,cluster_parameters=0,vectorizer_parameters=0,word_vec=0,num_clusters = 15,add_features = [],use_features =[],output_filename = "",print_all = True):
    ...
    def theme_cluster2(self,cluster_parameters=0,vectorizer_parameters=0,word_vec=0,num_clusters = 15,add_features = [],use_features =[],output_filename = "",print_all = True):
        from gensim.summarization import summarize  
        from sklearn.cluster import Birch
        from sklearn.feature_extraction.text import CountVectorizer
        import numpy as np
        ...
        for i,each in enumerate(self.dic):
            ...
            self.agg_temp = self.all_nf[i].agg(print_all = False,return_agg = True,write = True)
            self.cluster_agg.append(self.agg_temp)
            self.temp = Newfile(data = self.temp)
            # self.cluster_average = np.sum(self.temp.data["Vec"])/len(self.temp.data)
            # self.cluster_variance = (np.sum([np.square(self.cluster_average - each) for each in self.temp.data["Vec"]]))/len(self.temp.data)
