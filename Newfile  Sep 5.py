...
class Newfile():    
    def __init__(self,filename = "NO NAME.csv",data = 0,sent_col =0,content_col = 0,sheet = "sheet 1",ascii = False ):
        ...
        for each in list(self.data):
            if "Calculated Sentiment Subcategeory" in each:
                self.sub_sent_col = each
        # for each in list(self.data):
            # if "Calculated Sentiment Category" in each:
                # self.sent_col = each
    ...
    def csv(self,name= 0):
        ...
        self.data.to_csv(self.name,index = None)
        
    def agg_sub(self,agg_name = 0,target = 0,col = 0,sent = 0,preprocess= True,print_all = True):
        self.target = target
        self.agg_col = col
    ...
    def retag(self,key,all_rt = False):
        ...
        self.temp2 = 0
        return Newfile(filename = self.filename,data = self.data,content_col = self.content_col, sent_col = self.sent_col)
    ...
    def sentiment_sub(self,target_col = 0,write = False,print_all = True):
        ...
        self.sent_sum = {"Very Positive":str(round((self.tpx.shape[0]/(self.tpx.shape[0]+ self.tpsx.shape[0] + self.tnx.shape[0]+self.tnsx.shape[0]+ self.ttx.shape[0])*100),2)),
                "Somewhat Positive":str(round((self.tpsx.shape[0]/(self.tpx.shape[0]+ self.tpsx.shape[0] + self.tnx.shape[0]+self.tnsx.shape[0]+ self.ttx.shape[0])*100),2)),
                "Very Negative":str(round((self.tnx.shape[0]/(self.tpx.shape[0]+ self.tpsx.shape[0] + self.tnx.shape[0]+self.tnsx.shape[0]+ self.ttx.shape[0])*100),2)),
                "Somewhat Negative":str(round((self.tnsx.shape[0]/(self.tpx.shape[0]+ self.tpsx.shape[0] + self.tnx.shape[0]+self.tnsx.shape[0]+ self.ttx.shape[0])*100),2)),
                 "Neutral":   str(round((self.ttx.shape[0]/(self.tpx.shape[0]+ self.tpsx.shape[0] + self.tnx.shape[0]+self.tnsx.shape[0]+ self.ttx.shape[0])*100),2)),
                 "XX POSITIVE": str(round(((round((self.tpsx.shape[0]/(self.tpx.shape[0]+ self.tpsx.shape[0] + self.tnx.shape[0]+self.tnsx.shape[0]+ self.ttx.shape[0])*100),2)) + (round((self.tpx.shape[0]/(self.tpx.shape[0]+ self.tpsx.shape[0] + self.tnx.shape[0]+self.tnsx.shape[0]+ self.ttx.shape[0])*100),2))),2)),
                 "XX NEGATIVE": str(round(((round((self.tnsx.shape[0]/(self.tpx.shape[0]+ self.tpsx.shape[0] + self.tnx.shape[0]+self.tnsx.shape[0]+ self.ttx.shape[0])*100),2)) + (round((self.tnx.shape[0]/(self.tpx.shape[0]+ self.tpsx.shape[0] + self.tnx.shape[0]+self.tnsx.shape[0]+ self.ttx.shape[0])*100),2))),2))}
    ...
    def theme(self,key,trim = False,uncovered = True):
        ...
        for each_kw in range(len(self.theme_data)):
            ...
            if self.trim == True:
                self.op[[self.doc_col,"AuthorName","ChannelName",self.content_col,self.date_col,"SourceURL",self.sub_sent_col]].to_csv(r'theme//'+ self.t +" "+self.mainname,index=None)
            else:
                self.op.to_csv(r'theme//'+ self.t +" "+self.mainname,index=None)
    ...
    def generate_dates_key(self):
        ...
        self.data["p_date"] = [parse(each).strftime("%Y%m%d") for each in self.data[self.date_col].astype(str)]
    ...
    def get_trend_data3(self,sent = 0):
        try:
    ...
    def filter(self, col = 0,targ = 0, condition = "or",regex = False, value_filter = 0,name = 0,write = False,trim = False,return_uncov = False,exact_match = False):
        ...
        if self.write == True:
            self.temp.to_csv("Filtered "+self.filename,index = None)
    ...
    def ngrams(self,range = (1,2),num = 150,write = True,remove_handles = True,print_all = True,vectorizer_parameters = 0):
        ...
        self.ngram_data["s_content"] = self.ngram_data[self.content_col]
    ...
    def preprocess(self):
        ...
        # self.pp = self.data.copy()
        self.data["p_content"] = [re.sub("https://t.co/[a-zA-Z0-9_a ]{0,10}\S","",each) for each in self.data[self.content_col]]
...
#################################################################################################        
    ...
    def get_word_vec(self,word_vec_dim = 50):
        import numpy as np
...
#            self.data["p_content"] = self.data["p_content"]
        ...
        self.wv_model = Word2Vec(self.word_list, min_count=1,size= self.word_vec_dim)
    ...
    def theme_cluster2(self,cluster_parameters=0,vectorizer_parameters=0,word_vec=0,num_clusters = 15,add_features = [],use_features =[],output_filename = ""):
        from gensim.summarization import summarize  
        from sklearn.cluster import Birch
        from sklearn.feature_extraction.text import CountVectorizer
        import numpy as np
    ...
    def vector_variance_summarizer(self):
        ...
        try:
            ...
            self.sqd_i = self.sqd.sort_values().index[:200]
    ...
    def t_sentiment_function(self,text):
        ...
        tag = 0
        if sent_score < 0.3 and sent_score > -0.2:
            tag = "Neutral"
        elif sent_score >= 0.3 and sent_score < 0.7:
            tag = "Somewhat Positive"
        elif sent_score >= 0.7:
            tag = "Very Positive"
        elif sent_score <= -0.2 and sent_score > -0.7:
